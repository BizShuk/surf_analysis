...
- Wave analysis (optional): `surf extract <video> --wave [--wave-engine auto|ocean|static] [--view auto|facing|side]` adds per-frame `wave` + `wave_summary`, and bumps `schema_version` to `1.2` (render still reads `1.0` and `1.2`; **1.1 is rejected with `IncompatibleSchemaError`** per user decision 2026-06-21). `surf render` draws it unless `--no-wave`.
    ...
    - **Physical wave height in meters**（可選）:加 `--camera-height-m <H>` + (`--focal-length-mm` 或 `--sensor-height-mm`) 開啟。`H` 是相機至水面高度,focal/sensor 用於推算每個像素對應的世界深度。Top-down 場景 (e.g. sample.MOV) 還需要 `--camera-pitch-deg` — pitch 是 boolean gate,任何 > 0° 都能解鎖 projection。沒有 metadata 時,`wave_summary.physical_status="insufficient_metadata"`,`height_m_*: null`,CLI 在 stderr 印警告。詳見下方「Wave height semantics」一節與 `plans/2026-06-21-physical-wave-height-design.md`。
    - sample.MOV 實測 (camera_height_m=2.5, focal=26mm, sensor=4mm, pitch=45°):`height_m_median ≈ 2.94m`,`height_m_p90 ≈ 3.15m`,confidence=medium。Pitch 15°–90° 結果一致(`h = H × (1 - tan(α_crest)/tan(α_base))` 把 pitch 消掉)。絕對值依賴相機 metadata 準確度。
